...
export default function SidebarProject({ project, sessions, expanded, onToggle, currentSession, search = '' }: {
  ...
  return <div className="workbench-project">
    <div className="workbench-project-row">
      ...
      <Link to={`/?new=1&project=${encodeURIComponent(project.id)}`} className="workbench-nav-item" title={project.name}><Folder size={16} /><span className="truncate">{project.name}</span></Link>
    </div>
    {expanded && <div id={panelId} className="workbench-project-chats">
      {visible.map((session) => <Link key={session.id} to={chatHref(session.id)} className="workbench-nav-item workbench-session" aria-current={currentSession === session.id ? 'page' : undefined} title={session.title || session.id}><span className="truncate">{session.title || session.id}</span></Link>)}

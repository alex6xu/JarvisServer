# PRD: jarvis 对齐 pi Agent 的九项能力补齐

## Introduction

jarvis 是用 Go 复刻的 pi coding agent。当前 jarvis 已具备 agent loop、7 个内置工具（read/write/edit/bash/find/grep/ls）、多 provider（Anthropic/OpenAI/OpenRouter 等）、OAuth、hooks、skills、单进程 goroutine 子 agent、声明式 slash 命令、JSONL 会话持久化与 resume、headless/print/stream-json 模式。

本 PRD 覆盖 jarvis 相较原始 pi agent（`/Users/chaoyuepan/ai/pi/packages`）仍缺失的九项能力。九项分两类来源：

- **从 pi 移植**：#1 上下文压缩、#5 会话管理增强、#6 多模态输入、#7 编排器、#8 插件系统、#9 项目信任。这些在 pi 中有成熟实现，Go 版按其算法与数据结构落地。
- **对标 Claude Code 新增**：#2 MCP 客户端、#3 TODO 任务工具、#4 WebFetch/WebSearch。这三项 pi 本身没有（全局检索仅在 OAuth/vendor 命中），是 jarvis 为兼容 Claude Code 开放能力而新增。

面向读者可能是初级工程师或 AI agent，术语已尽量解释。

## Goals

- 让 jarvis 支持长会话不撑爆上下文窗口（自动 + 手动压缩）。
- 补齐 pi 已有的会话管理体验：导出、导入、fork、clone、分支树、复制、会话统计。
- 提供图像输入能力，支持把图片喂给多模态模型。
- 提供跨语言插件机制，让业务团队能定制专属工具/命令（贴合 jarvis "支持业务平台团队定制" 的定位）。
- 补齐 Claude Code 生态能力：MCP 客户端、TODO 任务追踪、联网抓取/搜索。
- 保留 jarvis "单进程 goroutine 子 agent" 的既有取舍，编排器仅把多进程隔离作为可选增强。
...
**Description:** 作为 jarvis，我想作为 MCP 客户端连接外部 MCP server，把其暴露的工具接入 agent 的工具集，兼容 Claude Code 生态。
...
**Description:** 作为业务团队，我想用任意语言写一个可执行文件作为插件，向 jarvis 注册自定义工具与命令，无需改动 jarvis 源码。

**Acceptance Criteria:**
- [ ] 新增 `internal/plugin` 包，定义插件协议：jarvis 以子进程启动插件可执行文件，走 stdio JSON-RPC（复用 US-014 的 JSON-RPC 基础设施）
- [ ] 握手阶段插件声明其提供的 tools（name/description/schema）与 slash commands
- [ ] 插件注册的工具被调用时，jarvis 通过 RPC 转发参数、回传结果
- [ ] 插件从配置目录（如 `~/.jarvis/plugins/`）自动发现
...
**Acceptance Criteria:**
- [ ] 插件握手时可声明订阅的事件类型；jarvis 在对应时机通过 RPC 通知插件
...
**Description:** 作为用户，我希望首次在某个项目目录运行 jarvis 时被询问是否信任该目录，信任决策被持久化，避免在不受信目录自动执行工具。

**Acceptance Criteria:**
- [ ] 新增 `internal/trust` 包，对标 pi trust-manager：信任存储为 `path → bool|null` 的 JSON 文件（`~/.jarvis/trust.json`）
...
- [ ] `process` 模式下 jarvis 以子进程启动自身的 headless/rpc 模式跑子 agent，通过 stdio JSON-RPC（复用 US-014 基础设施）转发 prompt 与回传最终结果
...
- 富 TUI（编辑器组件、fuzzy、kill-ring 等）——jarvis 已刻意移除（见 `prd-remove-tui.md`），维持行式 REPL。
- `/share`（GitHub gist 分享）、`/scoped-models`、themes 主题系统、telemetry 遥测、`/changelog`、`/reload` 热重载——不在本轮九项内。
- 与 pi 会话文件的 wire 级兼容——jarvis 自有 schema（见现有 session.go 说明）。
...
- **会话管理**（#5）：在现有 `internal/session` 上加 id/parentId 与迁移；`SchemaVersion` 已有版本钩子。命令加在 `internal/runtime/slashcommand.go` + `cmd/jarvis/repl.go`。
...
- MCP server 工具在 jarvis 中可被发现与调用，命名与 Claude Code 一致（`mcp__server__tool`）。
- 未信任目录下副作用工具默认需确认。

## Open Questions

- 压缩的 `KeepRecentTokens` 默认值取多少（pi 未在本次读取的片段中给出常量，spec 阶段定）？
- WebSearch 默认后端选哪个（自建 endpoint / 第三方 API），是否需要在 jarvis 内置一个默认？

...
`/btw`（by the way）为此提供一个**临时侧线程（side thread）**：用户可以在不改变主会话消息历史的前提下，向模型提一个或多个旁路问题。侧线程以**当前主会话的上下文作为背景**（模型能看到主对话，从而给出相关回答），但侧线程里的提问与回答**不会写回主会话、也不会落盘**——关闭侧线程、切换会话或重启 jarvis 后，这些内容全部丢弃。
...
- 支持**无参数重开**：`/btw`（不带问题）重新打开本次 jarvis 会话中最近一次侧线程，以便浏览之前的问答；若本会话尚无侧线程，则提示用户输入一个问题。
...
- [ ] Verify in a REPL（例如通过 `run` skill 启动 jarvis 交互式会话实测）。
...
**Acceptance Criteria:**
- [ ] 本次 jarvis 进程内**已有**至少一次 `/btw` 侧线程时，输入裸 `/btw` 重新打开最近一次侧线程，展示其历史问答，光标就绪可继续追问。
- [ ] 本次进程内**尚无**任何侧线程时，输入裸 `/btw` 提示用户输入一个问题（等同 US-001 的空线程起手），而非报错。
- [ ] 侧线程历史仅在**当前 jarvis 进程/会话**内保留；切换会话、重启 jarvis 后清空（不落盘）。
- [ ] Typecheck/lint 通过。
- [ ] Verify in a REPL。
...
- FR-6: 系统必须在整个 jarvis 进程内保留最近一次侧线程的消息，供裸 `/btw` 重开；切换会话或进程退出时丢弃。
...
- 侧线程 UI 参考 pi-btw：固定头部 `btw · side thread`，问答按时间从上到下滚动展示；页脚在可滚动时显示 `PgUp`/`PgDn` 提示，`Ctrl+C` 提示取消/退出。jarvis 当前 REPL 为逐行流式输出（非全屏 TUI），本期可先以**行式简化实现**：进入侧线程后打印头部横幅，逐轮流式打印问答，`Ctrl+C`/`/exit` 退出；`PgUp/PgDn` 滚动浏览为可选增强（见 Open Questions）。
...
- **拦截位置：** 在 `cmd/jarvis/repl.go` 主循环中，与 `/goal`（`repl.go:262`）同层新增 `if line == "/btw" || strings.HasPrefix(line, "/btw ")` 分支，调用新函数 `runBtw(setCancel, out, &deps, line)`。守卫需避免把 `/btweak` 之类误匹配（用 `== "/btw"` 或前缀 `"/btw "`）。
...
- **配置文件：** 对标 pi-btw 的 `pi-btw.json`，在 jarvis 中放置于 jarvis 配置目录（如 `$JARVIS_DIR/btw.json` 或 `~/.jarvis/btw.json`，最终路径以项目现有配置目录约定为准）。字段：`{"model": "<id>", "thinkingLevel": "off|minimal|low|medium|high|xhigh"}`。文件可选、不自动创建、每次调用读取。
...
- 行式实现 vs 全屏 overlay：jarvis 当前无全屏 TUI，`PgUp/PgDn` 滚动浏览是否本期实现，还是留作增强？
- 配置文件的确切路径与文件名需与项目现有配置目录约定对齐（`$JARVIS_DIR` 具体环境变量名待确认）。

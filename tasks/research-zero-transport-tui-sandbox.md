...
> 目的：为 jarvis 的 provider transport（US-007~013）、TUI（US-022）、sandbox（US-026）三个决策提供 Go 实现输入。
...
### A.2 关键设计（值得 jarvis 借鉴）

1. **transport/schema 分层**（最干净的部分）：一个 SSE scanner（`scanSSEPayloads` providerio.go:210，基于 `bufio.Scanner`，逐行累积 `data:`、blank line flush、丢 `[DONE]`、`:` 前缀当 keep-alive）；providers 只映射 schema。**jarvis 应直接采用这个分层** —— 正好填补地图上「Provider 通用 transport 抽象」雾区。
2. **错误在流内表达（dual failure model）**：pre-flight/marshal 错误走返回值 `error`（同步，channel 活动前）；**连接后的一切失败**（HTTP 非 2xx、坏 JSON、上游错误、idle/stall 超时）编码为终态 `StreamEventError` 事件再关 channel。**这正好印证 pi 的 `StreamFn` 契约**（FR-13：请求失败以流内终态 error/aborted 消息表达而非抛错）—— pi 和 zero 在这点上一致，jarvis 的 `Provider` 接口可放心照此设计，且可比 pi 更明确地区分「建流前错误=返回 error」与「建流后错误=流内事件」。
...
- **pi 的 StreamFn（不返回 error，失败进流）与 zero 的 dual model 一致**。jarvis `Provider` 接口建议：`StreamCompletion(ctx, req) (<-chan StreamEvent, error)`，返回的 error 仅限建流前失败；运行时失败进流内终态事件。这比 §研究1（pi 映射）§3.3 的「StreamFn 不返回 error」更精确，采纳 zero 的 dual 划分。
- **channel-based 流**与 §研究1 的 EventStream（chan）映射方向一致，互相印证。

### A.4 避免 / 重新考虑

- `DisableKeepAlives` 对整个 darwin 平台（providerio.go:171）过于粗暴（每请求全新 TCP+TLS）；jarvis 可用 per-conn 健康检查或 HTTP/2 替代。
...
- 四个适配器的 `stream()` 脚手架大量复制；jarvis 可抽成共享 driver + per-provider decoder。
...
- **对 jarvis 的直接意义**：jarvis 的 loop 发的 `AgentEvent`（§研究1 §2）在 TUI 模式下，正好通过一个 `func(AgentEvent)` sink → `program.Send` 桥接进 bubbletea。这解决了「loop 的 channel 事件流如何进 TUI 事件循环」这个 US-022 核心问题。
...
### B.4 对 jarvis 的建议（→ #11 TUI 选型决策的输入）
...
- **steering = queue-next-turn，非 mid-turn 注入**：agent 运行时提交的 prompt 存进 `m.queuedMessage`（单条），当前 run 结束后作为新 run 启动。
  - **与 pi 的差异（重要）**：pi 的 `getSteeringMessages` 是**每轮工具执行后注入到下一轮之前**（agent 还在干活时插话，同一 run 内）；zero 的 queue-next-turn 是**等整个 run 结束才启新 run**。pi 的 steering 更细粒度。jarvis 若要严格复刻 pi，TUI 层的 `getSteeringMessages` 应在**当前 run 内每轮后**喂入队列的消息，而非 zero 的「结束后启新 run」。**记录为 jarvis 需按 pi 语义实现、不照搬 zero 的点**。
...
**Layer A — 进程内策略引擎（always-on）**：`internal/sandbox/engine.go:277` `Evaluate(ctx, Request) Decision`，纯 Go 的 allow/prompt/deny 评估（路径校验、网络分类、破坏性命令检查、grant 查询），任何执行前跑。**这一层就是 jarvis v1 要的 `beforeToolCall` 门。**
...
**降级带底线**：OS backend 不可用时降到 `EnforcementDegraded/Disabled`，**但进程内策略门 + 逐命令审批仍生效**。—— **这正是 jarvis「OS 隔离作为后续步骤」要的语义：Go 门是地板，OS 隔离是叠加。**
...
- `internal/securefile`（AES-256-GCM 加密**存储**，仅 credstore 用）—— 是凭据落盘加密，非输出脱敏；jarvis 仅在持久化凭据时相关。
...
### C.5 对 jarvis 的建议（→ #7 sandbox 强度决策的输入）

**借鉴**：
- **解耦两层架构** —— `Evaluate(ctx, Request) → Decision{Action, Risk, Reason, Block}` 纯函数，就是 jarvis v1 的 `beforeToolCall` 门，独立于任何 OS backend 且可独立测试。
...
- **降级带底线**：OS 隔离不可用时保留进程内门 + 逐命令审批，绝不 fail-open。**直接匹配 jarvis「OS 隔离作后续」。**
...
- **不要混淆两个 "beforeTool"**：jarvis v1 sandbox 门单独就满足 `beforeToolCall`；通用外部 hook 系统是可分离的后续特性。
...
- **新发现的复刻差异**：zero 的 steering 是 queue-next-turn，**与 pi 的 per-turn steering 语义不同**；jarvis 须按 pi 实现（B.5）。这条影响 US-022 与 loop（US-006 已复刻 pi 语义，TUI 层对接时勿照搬 zero）。

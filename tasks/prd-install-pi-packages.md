# PRD: 安装 pi Agent 包（jarvis install）
...
jarvis 是 pi agent 的 Go 复刻版，已具备自己的 **plugin 系统**（`$JARVIS_HOME/plugins` 下的可执行文件，走 JSON-RPC）、**命令系统**（`$JARVIS_HOME/commands/*.md`）和 **skill 系统**（`~/.agents/skills`）。目前用户无法直接复用 pi 生态里的现成包。

本功能给 jarvis 增加一个 `jarvis install` 子命令，从 npm 拉取 pi 包，识别其类型，并把内容分发到 jarvis 对应的目录，使其被现有的发现机制自动加载。同时提供 `list` 与 `uninstall` 管理已安装的包。

**读者假设**：面向初级开发者或 AI agent，涉及的概念（npm 包结构、pi 包类型、jarvis 目录约定）首次出现时给出解释。

**关键决策（来自澄清问答）**：
- **获取方式（1B）**：调用本机已安装的 `npm`（`npm pack` / `npm install`）拉取包，不自己实现 npm registry 客户端。未检测到 npm 时给出清晰报错与安装指引。
- **包类型（2A）**：尽力映射全部 pi 包类型到 jarvis 对应机制。
...
- 用户可通过 `jarvis install npm:<package>` 一行命令安装 pi 生态的 npm 包。
- 安装时自动识别包类型并分发到 jarvis 对应目录，安装后无需额外配置即可被发现机制加载。
...
## pi 包类型 → jarvis 目标目录映射

| pi 包类型 | 识别依据（package.json 字段/约定） | jarvis 目标目录 | 说明 |
|-----------|-----------------------------------|--------------|------|
| extension（含 MCP adapter） | 声明可执行入口（`bin`/`pi.extension`） | `$JARVIS_HOME/plugins/` | 落地为可执行文件，走现有 JSON-RPC 插件协议 |
| skill | 含 `SKILL.md`/skill 元数据（`pi.skill`） | skills 目录（`~/.agents/skills` 或 `JARVIS_SKILLS_DIR`） | 复用现有 skill 加载 |
| prompt / command | 含命令模板（`pi.prompt` / `commands/*.md`） | `$JARVIS_HOME/commands/` | 复用现有命令加载 |
| theme | `pi.theme` | `$JARVIS_HOME/themes/`（暂存） | [Assumption] jarvis 尚无主题系统，先落盘保存，运行时消费留待后续（见 Open Questions） |
...
**Acceptance Criteria:**
- [ ] 在 `$JARVIS_HOME/packages.json`（或等价 lockfile）中记录每个已安装包：`name`、`source`（如 `npm:pi-mcp-adapter`）、`version`、`type[]`、安装落地的文件路径列表
...
**Description:** 作为用户，我希望 `jarvis install` 用本机 npm 把包下载并解包到临时目录，供后续分发。

**Acceptance Criteria:**
- [ ] 安装前检测 `npm` 是否在 PATH；缺失时报错 `npm not found; install Node.js/npm to use jarvis install` 并退出非零
...
**Description:** 作为用户，安装一个 extension（如 MCP adapter）后，它应作为 jarvis 插件被自动发现。

**Acceptance Criteria:**
- [ ] extension 包的可执行入口被落地到 `$JARVIS_HOME/plugins/`，且带可执行权限（chmod +x）
- [ ] 落地的文件路径写入 US-001 的 lockfile
- [ ] 安装后启动 jarvis，该插件出现在已加载插件中（现有 `plugin.Discover` 能发现它）
...
**Description:** 作为用户，安装一个 skill 包后，它应被 jarvis 的 skill 加载机制发现。

**Acceptance Criteria:**
- [ ] skill 包内容被落地到 skills 目录（`~/.agents/skills/<name>/`，遵循 `JARVIS_SKILLS_DIR` 覆盖）
- [ ] 落地文件路径写入 lockfile
...
**Acceptance Criteria:**
- [ ] command 模板被落地到 `$JARVIS_HOME/commands/`（遵循现有 `commands/*.md` 约定）
- [ ] 落地文件路径写入 lockfile
...
**Description:** 作为用户，安装 theme 包不应报错；即便 jarvis 暂无主题系统，也先把主题文件保存下来。

**Acceptance Criteria:**
- [ ] theme 包内容落地到 `$JARVIS_HOME/themes/<name>/`
- [ ] 落地文件路径写入 lockfile
- [ ] 安装成功输出提示：`theme installed but not yet consumed by jarvis runtime`（`[Assumption]`，见 Open Questions）
- [ ] 有测试验证 mock theme 包分发后出现在 themes 目录
- [ ] Typecheck/lint passes

### US-009: `jarvis install` 端到端命令
**Description:** 作为用户，我运行 `jarvis install npm:pi-mcp-adapter` 即可完成解析→拉取→识别→分发→写清单的全流程。

**Acceptance Criteria:**
- [ ] `jarvis install npm:<name>` 串联 US-002~US-008，成功后打印安装摘要（包名、版本、类型、落地目录）
- [ ] 任一步骤失败时报错清晰、退出非零，且不写入部分 lockfile 记录（要么全成功要么不留记录）
- [ ] 重复安装同一包时，先卸载旧版本文件再装新的（幂等，不残留旧文件）
- [ ] 有集成测试用 mock npm（或桩）验证一个 extension 包端到端安装
- [ ] Typecheck/lint passes

### US-010: `jarvis list` 列出已安装包
**Description:** 作为用户，我想查看已安装了哪些 pi 包及其版本和类型。

**Acceptance Criteria:**
- [ ] `jarvis list` 读取 lockfile，逐行打印 `name  version  type(s)  source`
- [ ] 无已安装包时打印 `no packages installed`
- [ ] 有单元测试覆盖有包/无包两种输出
- [ ] Typecheck/lint passes

### US-011: `jarvis uninstall` 卸载包
**Description:** 作为用户，我想卸载一个已安装的包并清理其落地文件。

**Acceptance Criteria:**
- [ ] `jarvis uninstall <name>` 删除该包在 lockfile 中记录的全部落地文件，并从 lockfile 移除该条目
...
### US-012: `jarvis update` 更新包到最新版本
**Description:** 作为用户，我想把已安装的包更新到 npm 上的最新版本。

**Acceptance Criteria:**
- [ ] `jarvis update <name>` 对指定包执行「卸载旧文件 → 按记录的 source 重新拉取最新版 → 重新分发 → 更新 lockfile 版本」
- [ ] `jarvis update`（不带包名）遍历更新 lockfile 中全部包
...
- FR-1: 系统必须提供 `jarvis install <source>` 子命令。
...
- FR-6: 系统必须把 extension（含 MCP adapter）类型落地到 `$JARVIS_HOME/plugins/` 并设置可执行权限。
- FR-7: 系统必须把 skill 类型落地到 skills 目录（遵循 `JARVIS_SKILLS_DIR` 覆盖）。
- FR-8: 系统必须把 prompt/command 类型落地到 `$JARVIS_HOME/commands/`。
- FR-9: 系统必须把 theme 类型落地到 `$JARVIS_HOME/themes/` 并提示暂未被运行时消费。
- FR-10: 系统必须在 `$JARVIS_HOME/packages.json` 中记录每个已安装包的 name、source、version、type[]、落地文件列表。
- FR-11: 系统必须提供 `jarvis list` 列出 lockfile 中的已安装包。
- FR-12: 系统必须提供 `jarvis uninstall <name>` 删除包的落地文件并移除 lockfile 条目。
- FR-13: 系统必须提供 `jarvis update [name]` 将指定包（或全部包）更新到最新版本。
...
- 不实现 jarvis 的主题运行时系统；theme 包仅落盘暂存。
...
- 复用现有发现机制：`plugin.Discover($JARVIS_HOME/plugins)`、skills 加载（`~/.agents/skills` / `JARVIS_SKILLS_DIR`）、命令加载（`$JARVIS_HOME/commands/*.md`）。分发只需把文件放对目录，无需改动加载器。
- `$JARVIS_HOME` 解析沿用现有约定（`JARVIS_HOME` 环境变量，默认 `~/.jarvis`）。
- 调用 npm 时用非交互方式（避免交互式提示阻塞），透传 stderr 以便诊断。
- lockfile 与安装模块建议置于新 package（如 `internal/pkgmgr`），CLI 子命令置于 `cmd/jarvis`。
- 安全提示：安装第三方可执行 extension 会在本机运行外部代码，属于中高风险动作；安装 extension 时应向用户提示来源与风险（可结合已有的 Project Trust 机制，见近期提交 US-018）。

## Success Metrics

- 用户可用单条 `jarvis install npm:pi-mcp-adapter` 完成安装，安装后重启 jarvis 即可发现该插件（0 额外手动步骤）。
- `list`/`uninstall`/`update` 对 lockfile 的操作 100% 一致（uninstall 后 list 不再显示该包，磁盘无残留记录文件）。
- 安装失败场景（无 npm、包不存在、类型无法识别）均给出可操作的明确报错，无半安装残留。

## Open Questions

- extension 若是 node 脚本入口（非原生可执行），jarvis 插件协议要求可执行文件——需确定是落地一个调用 `node xxx.js` 的 wrapper 脚本，还是要求包提供原生可执行？（影响 US-005）

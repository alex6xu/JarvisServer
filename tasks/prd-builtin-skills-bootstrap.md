...
jarvis 目前的 skills 全部依赖用户手动安装（`jarvis install npm:...` 或把目录放进 `~/.agents/skills`）。新用户开箱后没有任何可用的工作流技能，需要自己去 goal.rpcx.io 逐条安装，门槛高。

本功能在 jarvis **首次运行**时自动把一批"内置 skills"落地到用户的 skills 目录（`~/.agents/skills`，可被 `JARVIS_SKILLS_DIR` 覆盖），使 `/prd`、`/to-issues`、`/refactor` 等命令开箱即用。

技术路线（已确认）：
- **1A** — skills 内容通过 Go `embed` 打进 jarvis 二进制，首次运行时复制到磁盘。完全离线、无外部依赖（不依赖 `npx`/`gh`/网络）。
- **2A（去掉 humanize-it）** — 内置 goal-workflow 全套技能（**排除 `humanize-it`**），另加 `architecture-diagram`、`weather` 两个独立技能。
- **3C** — 首次运行**静默安装、无提示、无输出**。
- **4A** — 失败时优雅降级：记录一条 warning（或静默跳过），jarvis 正常启动，下次运行再重试。
...
> 注：`/goal` 是 Claude Code / jarvis 内置能力，不属于本清单。`weather` 运行时依赖 `curl`；`architecture-diagram` 生成 HTML+SVG，无外部依赖。

## Goals

- 新用户首次运行 jarvis 后，goal-workflow 内置技能立即以 `/skill-name` 形式可用，无需任何手动步骤。
- 安装完全离线，不依赖 `npx`、`gh` 或网络。
- 首次运行安装过程静默（无提示、无输出），不打断用户。
- 安装失败不阻塞 jarvis 启动；下次运行自动重试。
...
**Description:** As a jarvis maintainer, I want the goal-workflow skills embedded into the binary so they ship with jarvis and install offline.
...
### US-002: 首次运行判定与状态标记
**Description:** As a developer, I need jarvis to detect "first run" reliably so bootstrap happens exactly once and can retry after failure.

**Acceptance Criteria:**
- [ ] 在 `$JARVIS_HOME`（默认 `~/.jarvis`）下用一个 sentinel 文件（如 `builtin-skills.state`，记录已安装的技能集版本/清单）判定是否已 bootstrap
- [ ] sentinel 不存在或记录的清单版本落后于当前二进制 → 触发 bootstrap
- [ ] bootstrap **成功后**才写入/更新 sentinel；失败则不写，保证 4A 的"下次重试"
- [ ] `$JARVIS_HOME` 不可解析时（Home() 返回 ""）跳过 bootstrap 且不报错
...
**Acceptance Criteria:**
- [ ] 把每个内置技能复制到 `SkillsDir()/<name>/`（`JARVIS_SKILLS_DIR` 优先，否则 `~/.agents/skills`），保持 `SKILL.md` 及支撑文件的目录结构
...
- [ ] 单测：向临时 `JARVIS_SKILLS_DIR` 安装后，目标目录出现 16 个技能子目录，且每个含 `SKILL.md`
...
**Acceptance Criteria:**
- [ ] 在 REPL 启动路径（`dispatch`/`runInteractive`，`cmd/jarvis/run.go`）中，加载 skill 命令**之前**调用 bootstrap
- [ ] 正常路径：无任何 stdout 提示、无交互（3C）
- [ ] 失败路径（如 skills 目录不可写）：jarvis 继续正常启动进入 REPL；错误仅在 `JARVIS_DEBUG`/verbose 下打印，正常模式静默（4A）
...
- FR-3: 系统必须在 `$JARVIS_HOME` 下维护一个 sentinel 状态文件，用于判定是否已完成 bootstrap 及所安装的清单版本。
...
- FR-9: bootstrap 失败时，系统必须让 jarvis 正常启动，仅在 debug/verbose 下输出错误。
...
- 不接管或改动现有 `jarvis install/uninstall/update` 的 npm 安装路径。
...
- 复用已有基础设施：
  - `pkgmgr.SkillsDir()`（`internal/pkgmgr/layout.go`）解析 skills 目录，含 `JARVIS_SKILLS_DIR` 覆盖。
  - `pkgmgr.Home()` 解析 `$JARVIS_HOME`（默认 `~/.jarvis`）作为 sentinel 存放位置。
  - `DistributeSkill` 的复制模式（`copyTree`）可参考；但内置安装是"已存在则跳过"，与 `DistributeSkill` 的"先 RemoveAll 再覆盖"语义不同，需独立实现或加参数区分。
  - `loadSkillCommands()` / `buildSlashRegistry`（`cmd/jarvis/run.go`、`skills_test.go`）负责把 skills 目录暴露为 `/skill-name`，bootstrap 必须在其之前执行。
...
- 全新环境（空 `~/.jarvis` 与空 `~/.agents/skills`）首次运行 jarvis 后，`/prd`、`/refactor`、`/architecture-diagram`、`/weather` 等 16 个命令可直接调用。

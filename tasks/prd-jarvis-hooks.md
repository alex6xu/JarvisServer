# PRD: jarvis Hooks(用户可扩展的生命周期钩子)

## 1. Introduction/Overview

为 jarvis 增加一套**用户可扩展的 Hook(钩子)系统**,借鉴 Claude Code 的 hook 思想:用户在配置文件中声明 shell 命令,jarvis 在 agent 生命周期的关键节点触发这些命令,把事件数据通过 stdin 以 JSON 传给命令,并读取命令的退出码与 stdout 来决定后续行为。

jarvis 目前已经有两类"内部"扩展能力:Go 层的函数钩子(`internal/agenttool/tool_executor.go` 的 `BeforeToolCall`/`AfterToolCall`、`internal/runtime/loop.go` 的 `ShouldStopAfterTurn` 等),以及一条单向的插件事件总线(`internal/plugin/events.go`,插件只能"观察"事件,不能改变行为)。二者都不能让**普通用户在不写 Go、不编译插件的前提下**注入自己的逻辑。本特性填补这个空白:让用户仅靠配置 + 一段 shell 脚本,就能在工具执行前拦截、在用户提交提示词时注入上下文、在 agent 结束前阻止退出等。

目标读者假设为初级开发者或 AI agent,因此本文尽量用平实语言并给出可验证的判定标准。

## 2. Goals

- 让用户**无需写 Go、无需编译**,仅通过配置文件中的 shell 命令即可扩展 jarvis 行为。
...
**Description:** As a jarvis 用户, I want 在 config.json 中声明 hooks so that 我可以把 shell 命令绑定到生命周期事件上而无需改动 jarvis 源码。
...
### US-002: Hook 匹配引擎
**Description:** As a jarvis 用户, I want 用 matcher 精确指定哪些事件触发哪个 hook so that 我可以只对特定工具(如 `bash`、`write`)挂钩,而不影响其它工具。
...
**Description:** As a jarvis 用户, I want jarvis 把事件数据以 JSON 通过 stdin 传给我的 shell 命令 so that 我的脚本能读取上下文(工具名、参数、session_id、cwd 等)做判断。
...
- [ ] 注入环境变量:至少 `JARVIS_SESSION_ID`、`JARVIS_PROJECT_DIR`、`JARVIS_EVENT_TYPE`
- [ ] payload 只包含**可观测、非敏感**字段(对齐现有 `events.go` 的 "never secrets" 纪律),明确不包含 API key/凭证
- [ ] 命令 cwd 默认为项目工作目录
- [ ] 单元测试:用一个把 stdin 回显到临时文件的脚本,断言 jarvis 传入的 JSON 结构正确
- [ ] Typecheck/lint 通过

### US-004: Hook 决策协议(退出码 + JSON stdout)
**Description:** As a jarvis 用户, I want 用退出码和 stdout JSON 告诉 jarvis "放行/阻断/注入什么" so that 我的 hook 能真正影响 agent 行为。
...
**Description:** As a jarvis 用户, I want 在工具执行前用 hook 拦截或修改参数 so that 我可以阻止危险命令(如 `rm -rf`)或强制规范化参数。
...
**Description:** As a jarvis 用户, I want 在工具执行后对结果做检查并给模型反馈 so that 我可以在写文件后自动跑 lint 并把结果反馈给 agent。
...
**Description:** As a jarvis 用户, I want 在我提交提示词时自动注入上下文或拦截 so that 我可以自动附加当前 git 分支、时间戳,或拦截含敏感词的输入。
...
**Description:** As a jarvis 用户, I want 在 agent 打算结束时用 hook 决定是否强制它继续 so that 我可以要求 agent 在未完成 checklist 前不停止。
...
**Description:** As a jarvis 用户, I want 在子 agent 完成时触发 hook so that 我可以聚合或校验子 agent 的产出。
...
**Description:** As a jarvis 用户, I want 在会话开始时触发 hook so that 我可以加载额外上下文(如从外部系统拉取当前任务)。
...
**Description:** As a jarvis 用户, I want 在会话结束时触发 hook so that 我可以做清理、归档会话或上报统计。
...
**Description:** As a jarvis 用户, I want 在上下文压缩前触发 hook so that 我可以在信息被摘要前先做备份或自定义保留策略。
...
**Description:** As a jarvis 用户, I want 在 jarvis 需要我关注(如信任闸门要求确认副作用工具)时触发 hook so that 我可以把通知转发到桌面/IM。
...
**Description:** As a jarvis 用户, I want hook 出错时不影响主流程 so that 一个写坏的 hook 不会让整个 agent 崩溃或卡死。
...
**Description:** As a jarvis 用户, I want 清晰的文档和可复制的示例 so that 我能快速写出第一个 hook。
...
- FR-4: 系统必须在触发 hook 时,将事件 payload 以 JSON 从 stdin 传入命令,并注入 `JARVIS_SESSION_ID`/`JARVIS_PROJECT_DIR`/`JARVIS_EVENT_TYPE` 环境变量。
...
- FR-14: 系统必须仅在**受信任项目**中启用项目级(`.jarvis`)hooks;用户级 hooks 始终启用。
...
- shell 执行需跨平台(darwin/linux;Windows 若支持则用对应 shell),命令注入风险由"用户自写命令"承担,但 jarvis 传入 stdin 的 payload 必须是合法转义的 JSON。
...
- 是否需要在 v1 提供 `jarvis hooks list` / `jarvis hooks test <event>` 之类的调试子命令?(建议 v1 可选,v1.1 补齐)
